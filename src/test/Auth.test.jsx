...
vi.mock("../styles/modules/SignInBanner.module.css", () => ({ default: { signInBanner: "signInBanner" } }));
vi.mock("../styles/modules/SearchBar.module.css", () => ({ default: { searchbar: "searchbar", icon: "icon" } }));
vi.mock("../styles/modules/Playlist.module.css", () => ({ default: { playlist: "playlist" } }));
vi.mock("../styles/modules/SearchResults.module.css", () => ({ default: { searchResults: "searchResults" } }));
vi.mock("../styles/modules/Tracklist.modules.css", () => ({ default: { tracklist: "tracklist" } }));
...
describe("Authentication Flow", () => {
	...
	it("shows sign in banner when not logged in", async () => {
		...
		global.fetch = vi.fn().mockResolvedValue(mockFetchResponse({ access_token: "public_token" }));
	...
	it("attempts to refresh token if spotify_logged_in is true in localStorage", async () => {
		...
		global.fetch = vi.fn()
			...
			.mockResolvedValueOnce(mockFetchResponse({ access_token: "public_token" }));

		render(<App />);

		await waitFor(() => {
			expect(global.fetch).toHaveBeenCalledWith("/api/spotify-auth", expect.anything());
		});
	});
	...
	it("exchanges code for token when 'code' param is in URL", async () => {
		...
		await waitFor(() => {
			expect(global.fetch).toHaveBeenCalledWith("/api/spotify-auth", expect.objectContaining({
				method: "POST",
				body: JSON.stringify({
